  ...
  try {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        undoStack: Array.isArray(ratingSession.undoStack)
          ? ratingSession.undoStack.slice(-40)
          : [],
      })
    );
  } catch {
    ...
    <div class="rating-page">
      ...
      <article class="card rating-card">
        ...
        <div class="rating-actions form-actions">
          <button type="button" class="ghost" id="rating-undo" ${
            !(ratingSession.undoStack && ratingSession.undoStack.length)
              ? 'disabled'
              : ''
          } title="Undo last next or skip">
            Undo
          </button>
...
function wireRatingSong(gen, song, yt) {
  ...
  function pushRatingUndo() {
    if (!ratingSession) return;
    if (!Array.isArray(ratingSession.undoStack)) ratingSession.undoStack = [];
    ratingSession.undoStack.push({
      index: ratingSession.index,
      draft: ratingSession.draft,
      finished: ratingSession.finished,
      entries: ratingSession.entries.map((e) => ({
        song: e.song,
        rating: e.rating,
      })),
    });
    // Cap stack so memory stays light
    while (ratingSession.undoStack.length > 80) ratingSession.undoStack.shift();
  }

  function applyRatingUndo() {
    if (!ratingSession?.undoStack?.length) return false;
    const snap = ratingSession.undoStack.pop();
    if (!snap) return false;
    ratingSession.entries = snap.entries.map((e) => ({
      song: e.song,
      rating: e.rating,
    }));
    ratingSession.index = snap.index;
    ratingSession.draft =
      snap.draft != null && Number.isFinite(snap.draft)
        ? clampRating(snap.draft)
        : snap.entries[snap.index]?.rating != null
          ? clampRating(snap.entries[snap.index].rating)
          : null;
    ratingSession.finished = Boolean(snap.finished);
    return true;
  }

  document.getElementById('rating-undo')?.addEventListener('click', () => {
    if (!ratingSession) return;
    if (!applyRatingUndo()) return;
    disposeMedia({ removeTransition: true });
    saveRatingSession();
    render();
  });
...
/**
 * Load an image for canvas export (handles CORS failures with a null).
 * Spotify/YT CDNs often allow anonymous; if not, we draw a fallback tile.
 */
...
/**
 * Paint a shareable PNG of the ranked grid (album art + score overlays).
 * @returns {Promise<Blob>}
 */
async function buildRatingResultsImageBlob(ranked, title) {
  ...
  // 2x for crisp exports on retina
  const scale = 2;
  ...
  ctx.textAlign = 'left';
  ctx.fillText('Rating Mode', pad, pad + 28);
  ...
  for (let i = 0; i < n; i++) {
    ...
    const e = ranked[i];
    const img = images[i];
    const score = formatRating(e.rating);
    ...
    // Dark gradient for score readability
    ...
    ctx.shadowBlur = 12;
    ctx.fillText(score, x + cell / 2, y + cell / 2 + 6);
    ctx.shadowBlur = 0;
    ...
    const titleText = e.song.name || 'Unknown';
    let t = titleText;
  ...
  ctx.font = '500 11px system-ui, sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText('Playlist Bracket', width - pad, height - 12);
...
async function copyOrDownloadRatingImage(blob, btn) {
  ...
  try {
    ...
    a.href = url;
    a.download = `rating-results-${Date.now()}.png`;
    a.click();
...
function renderRatingResults(gen) {
  ...
  app.innerHTML = `
    <div class="rating-page rating-results-page">
      ...
      <div class="form-actions rating-results-actions">
        <button type="button" class="ghost" id="rating-undo-final" ${
          !(session.undoStack && session.undoStack.length) ? 'disabled' : ''
        }>Undo last</button>
        <button type="button" class="ghost" id="rating-export">Copy text</button>
        <button type="button" id="rating-export-image">Copy image</button>
        <button type="button" class="ghost" id="rating-again">Rate again</button>
  ...
  document.getElementById('rating-export-image')?.addEventListener('click', async () => {
    ...
    try {
      ...
      await copyOrDownloadRatingImage(blob, btn);
    } catch {
  ...
  document.getElementById('rating-undo-final')?.addEventListener('click', () => {
    if (!ratingSession?.undoStack?.length) return;
    const snap = ratingSession.undoStack.pop();
    if (!snap) return;
    disposeMedia({ removeTransition: true });
    ratingSession.entries = snap.entries.map((e) => ({
...
function onMatchKeydown(e) {
  ...
  if (uiMode === 'rating' && ratingSession && !ratingSession.finished) {
    ...
    if (key === 'u' || key === 'U') {
      e.preventDefault();
      document.getElementById('rating-undo')?.click();
      return;
    }
    return;
  }
